 ...
 * @author ivan
 */
public class ModelZadacha1 extends AbstractTableModel{
    List<LSZadacha> list = new ArrayList<>();
    List<String> listCol = new ArrayList<>();

    int vid;
    
    public ModelZadacha1(int v) {
        vid = v;
        listCol.add("№");        
        listCol.add("Статус");  
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        switch(columnIndex)
        {
            ...
            case 11: 
                return list.get(rowIndex).getPmh();        
    ...
    /*public void setData()
    {
        list = IOZadacha.getListLSZadacha(-1, vid);
        fireTableDataChanged();
    }*/
    
    public void setData(Date d1, Date d2, int hand)
    {
        list = IOZadacha.getListLSZadacha(vid, d1, d2, hand);
        fireTableDataChanged();
    }
   ...
   /* public void setDataForCreateNastils(Date d1, Date d2, int hand, boolean is_vizible_close)
    ...
    public DefaultTableCellRenderer jTableRendererHand = new DefaultTableCellRenderer() {
     ...
     private ImageIcon pln = new ImageIcon(getClass().getResource("/org/example/images/work.png"));
    public DefaultTableCellRenderer jTableRendererPh = new DefaultTableCellRenderer() {
        @Override
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
